module overmind::NonFungibleToken {
    ...
    */
    public fun mint_nft(
        ...
        assert!(coin::value(payment_coin)>= price, EInsufficientPayment);
        ...
        event::emit(NonFungibleTokenMinted{
            nft_id: object::uid_to_inner(&new_id),
            recipient: recipient,
        });
        ...
        transfer::transfer(new_nft, recipient);
    ...
    */
    public fun combine_nfts(
        ...
        let new_name = name(&nft1);
        string::append_utf8(&mut new_name, b" + ");
        string::append(&mut new_name, name(&nft2));
        let b_new_name = string::bytes(&new_name);



        let new_description = string::utf8(b"Combined NFT of ");
        string::append(&mut new_description, name(&nft1));
        string::append_utf8(&mut new_description, b" and ");
        string::append(&mut new_description, name(&nft2));
        let b_new_description = string::bytes(&new_description);
        let new_id = object::new(ctx);
        ...
        event::emit(NonFungibleTokenCombined{
            nft1_id: id1,
            nft2_id: id1,
            new_nft_id: object::uid_to_inner(&new_id),
        });
    ...
    */
    public fun withdraw_sales(
        ...
        // std::debug::print(&balance::value(&minter_cap.sales)); // Before withdrawal
        let to_withdraw = &mut minter_cap.sales;
        let amount = balance::value(to_withdraw);
        let change_coin = coin::take(to_withdraw, amount, ctx);
        // std::debug::print(&balance::value(to_withdraw));  // After withdrawal
        ...
        // let to_withdraw = &mut minter_cap.sales;
        // let amount = balance::value(to_withdraw);
        // coin::take(to_withdraw, amount, ctx)
    }
    ...
    */
    ...
    fun test_combine_nfts_success_1() {
        ...
        std::debug::print(&string::utf8(b"02"));
        assert_eq(
            test_scenario::num_user_events(&tx), 
            expected_events_emitted
        );
